...
public class MensagemService {
	...
	@Scheduled(fixedDelay=1000 * 60)// a cada minuto desde a inicializacao
	public void contarUltimasPostagens() {
		LocalDateTime agora = LocalDateTime.now();
		Long num = mensagemRepository.contarPostagensEmIntervaldoDeTempo(agora.minusMinutes(1), agora);
		System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
		System.out.println("UFMA-FORUM >>> Total de postagens no último minuto: "+num);
		System.out.println("UFMA-FORUM >>> Entre "+agora.minusMinutes(1)+" e "+agora);
		System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
	}
